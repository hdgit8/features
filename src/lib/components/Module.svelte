<script lang="ts">
    ...
    async function loadSections() {
        const{data} = await supabase.from("sections").select().eq("course_id", courseId).eq("module_id", id)
        if (data)
        {
            sections = data;
        }
...
<button class="module bg-[--dark-800]" on:click>
    <div class="flex p-3 font-semibold text-lg">
    ...
    {#if sections.length}
        <div class="p-3 bg-[--dark-900] flex flex-col gap-2">
            {#each sections as section(section.id)}
                <Section on:update={sortSections} {supabase} courseId={courseId} bind:moduleNum={num} id={section.id} bind:name={section.name} bind:num={section.num}></Section>
            {/each}
        </div>
    {/if}
</button>
